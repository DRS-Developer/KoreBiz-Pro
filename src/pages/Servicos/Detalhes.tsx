...
const ServicosDetalhes: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchService = async (slugValue: string) => {
        try {
            setLoading(true);
  ...
  const pageTitle = service?.title || 'Carregando...';

  return (
    <div className="flex flex-col min-h-screen">
      <SEO 
        title={pageTitle} 
        description={service?.short_description || `Saiba mais sobre ${pageTitle}`}
        image={service?.image_url || undefined}
      />
      {/* Header */}
      <PageHeader title={pageTitle}>
        <Link to="/servicos" className="text-blue-100 flex items-center mb-0 text-sm hover:text-white transition-colors">
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:w-2/3">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 mb-8 min-h-[200px]">
              {service ? (
                <>
          ...
          <div className="lg:w-1/3 space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-24">
              ...
              <p className="text-gray-600 mb-6 text-sm">
                Interessado em {service?.title || 'nossos serviços'}? Preencha o formulário abaixo e entraremos em contato rapidamente.
              </p>
              
              <Link 
                to="/contato" 
                className="block w-full bg-green-500 text-white font-bold py-3 px-4 rounded-lg text-center mb-4"
              >
                Falar no WhatsApp
              </Link>
